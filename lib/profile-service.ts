...
export class ProfileService {
  ...
  /**
   * Get a profile by username from Supabase database
   */
  static async getProfileByUsernameFromDatabase(username: string): Promise<UserProfile | null> {
    try {
      const supabase = getSupabaseClient()
      ...
      if (error || !profile) {
        console.log(`Profile not found in database: ${username}`)
        return null
      }
      ...
      console.error('Error fetching profile by username from database:', error)
      return null
    }
  }

  /**
   * Get a profile by username from localStorage (DEPRECATED - use getProfileByUsernameFromDatabase)
   */
